...
- **Maintenance menu (v0.5.0), in `app_maint.go` + `internal/<engine>/maint.go`.** Native **Maintenance** menu for server administration: users/permissions, create database, server activity (process list + locks + kill), and cross-engine maintenance ops (OPTIMIZE/ANALYZE/VACUUM/integrity check/REINDEX). Optional capability interface `engine.MaintenanceEngine` — type-asserted from `a.conn`; SQLite implements only `RunMaintenance` + `Capabilities()`. Admin SQL is built in `internal/maint` with `sqlbuilder.ValidateIdentifier` — never raw user input in SQL. All writes gated on `license.FeatModifySchema` (Pro). Menu emits `menu:maint-users`, `menu:maint-database`, `menu:maint-activity`, `menu:maint-tools`; frontend dialogs live in `frontend/src/components/maintenance/`.

## Out of scope — do not build (PRD §4)

Even if they look like trivial extensions: any non-macOS OS, SSL/TLS options, master password, FK navigation, command palette, schema diff, mysqldump backup, views/triggers/procedures management, **database deletion from UI**, visual query builder, auto-update, code signing, telemetry, multi-user/cloud sync, i18n, persistent query history. No ORMs (GORM/ent) — use `database/sql` + `go-sql-driver/mysql` directly.
...
> **Note (v0.5.0):** PRD §4 originally excluded "DB create/delete from UI"; **create database** and **user/role management** are now in scope via the Maintenance menu (PRD §17). Database **deletion** from UI remains out of scope.
